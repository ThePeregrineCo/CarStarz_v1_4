-- =============================================
-- CARSTARZ SIMPLIFIED DATABASE SCHEMA
-- =============================================
-- This script sets up the database schema for the CarStarz application
-- with a focus on the identity-wallet relationship.
--
-- Key features:
...
BEGIN
    ...
    result := 'Setup verification: ' || table_count || ' tables created. ';
    
    IF admin_count > 0 THEN
        result := result || 'Admin user created successfully.';
    ELSE
        result := result || 'Warning: No admin user found.';
    END IF;
