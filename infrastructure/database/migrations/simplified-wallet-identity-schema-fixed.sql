-- =============================================
-- CARSTARZ SIMPLIFIED DATABASE SCHEMA (WITH TOKEN OWNERSHIP FIX)
-- =============================================
-- This script sets up the database schema for the CarStarz application
-- with a focus on the identity-wallet relationship and ensures token ownership
-- is properly created and populated.
--
-- Key features:
-- 1. Stores wallet addresses only in identity_registry table
-- 2. Uses owner_id in vehicle_profiles to link to identity_registry
-- 3. Handles NFT transfers by updating owner_id
-- 4. Implements Row-Level Security (RLS) for authorization
-- 5. ENSURES token_ownership table is created and populated
--
-- You can run this entire script at once, or run sections individually
-- by selecting the relevant portion in the Supabase SQL Editor.

-- =============================================
-- SECTION 1: CLEANUP - DROP EXISTING TABLES
-- =============================================
-- This section drops all existing tables to start with a clean slate.
-- You can skip this section if you want to preserve existing data.
...
DROP TABLE IF EXISTS token_ownership CASCADE; -- Ensure this is dropped properly
...
-- =============================================
-- SECTION 2: SETUP - CREATE EXTENSIONS AND UTILITY FUNCTIONS
-- =============================================
...
-- =============================================
-- SECTION 3: CORE TABLES - IDENTITY AND VEHICLES
-- =============================================
...
-- =============================================
-- SECTION 4: VEHICLE DETAIL TABLES
-- =============================================
...
-- =============================================
-- SECTION 5: BLOCKCHAIN INTEGRATION TABLES
-- =============================================
...
-- =============================================
-- SECTION 6: VERIFICATION SYSTEM TABLES
-- =============================================
...
-- =============================================
-- SECTION 7: SOCIAL FEATURES TABLES
-- =============================================
...
SELECT
    ...
    vp.year,
    vp.name AS vehicle_name,
    from_ir.wallet_address AS from_wallet,
    from_ir.username AS from_username,
    to_ir.wallet_address AS to_wallet,
    to_ir.username AS to_username,
    ot.transaction_hash,
    ot.block_number,
    ot.transfer_date
FROM
    ownership_transfers ot
JOIN
    vehicle_profiles vp ON ot.vehicle_id = vp.id
JOIN
    identity_registry to_ir ON ot.to_identity_id = to_ir.id
LEFT JOIN
    identity_registry from_ir ON ot.from_identity_id = from_ir.id
ORDER BY
    ot.transfer_date DESC;
...
-- =============================================
-- SECTION 14: TOKEN OWNERSHIP VERIFICATION
-- =============================================
