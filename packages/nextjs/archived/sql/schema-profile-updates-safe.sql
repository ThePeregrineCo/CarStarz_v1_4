...
-- Club vehicles table - MODIFIED to use a safer approach with token_id
CREATE TABLE IF NOT EXISTS club_vehicles (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  club_id UUID REFERENCES clubs(id) NOT NULL,
  token_id INTEGER NOT NULL, -- Using token_id instead of vehicle_id foreign key
...
-- Builder vehicles table - NEW table for tracking builder-vehicle relationships
...
CREATE INDEX IF NOT EXISTS idx_builder_vehicles_business_id ON builder_vehicles(business_id);
CREATE INDEX IF NOT EXISTS idx_builder_vehicles_token_id ON builder_vehicles(token_id);
