  ...
  try {
    ...
    const { data: business, error: businessError } = await client
      ...
      .upsert([{
        ...
        location: 'Detroit, MI'
      }], { onConflict: 'user_id' }) // Changed to just user_id for safer conflict handling
      .select()
      .single();
    ...
    // Add a test builder-vehicle relationship
    console.log('Adding test builder-vehicle relationship...');
    const { error: builderVehicleError } = await client
      .from('builder_vehicles')
      .insert({
        business_id: business.id,
        token_id: 1, // Assuming token ID 1 exists or will exist
        work_type: 'Engine Swap',
        build_description: 'Complete LS3 engine swap with custom headers and ECU tune',
        build_date: new Date().toISOString()
      });
      
    if (builderVehicleError) {
      console.error('Error adding builder-vehicle relationship:', builderVehicleError);
    }
    ...
    const { data: club, error: clubError } = await client
      ...
      }], { onConflict: 'creator_id' }) // Changed to just creator_id for safer conflict handling
      .select()
      .single();
    ...
    // Add a test club-vehicle relationship
    console.log('Adding test club-vehicle relationship...');
    const { error: clubVehicleError } = await client
      .from('club_vehicles')
      .insert({
        club_id: club.id,
        token_id: 1, // Assuming token ID 1 exists or will exist
    ...
    if (clubVehicleError) {
      console.error('Error adding club-vehicle relationship:', clubVehicleError);
    }
